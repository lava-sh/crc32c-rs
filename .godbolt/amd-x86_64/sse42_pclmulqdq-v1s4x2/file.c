...
CRC_EXPORT uint32_t crc32_impl(uint32_t crc0, const char* buf, size_t len) {
  ...
  if (len >= 80) {
    ...
    crc2 = _mm_crc32_u64(crc2, *(const uint64_t*)(buf + klen * 2 + 8));
    crc3 = _mm_crc32_u64(crc3, *(const uint64_t*)(buf + klen * 3 + 8));
    vc0 = crc_shift(crc0, klen * 3 + blk * 16);
    vc1 = crc_shift(crc1, klen * 2 + blk * 16);
    vc2 = crc_shift(crc2, klen + blk * 16);
    vc3 = crc_shift(crc3, 0 + blk * 16);
    vc = _mm_extract_epi64(_mm_xor_si128(_mm_xor_si128(vc0, vc1), _mm_xor_si128(vc2, vc3)), 0);
