...
pub unsafe fn crc32c(mut crc0: u32, mut buf: *const u8, mut len: usize) -> u32 {
    ...
    if (256..=3072).contains(&len) {
        return !unsafe { super::small_sse42_pclmulqdq::crc32c_small(crc0, buf, len) };
    }
    ...
    if len >= 384 {
        ...
        let mut x3 = unsafe { _mm512_loadu_si512(buf2.add(192).cast()) };
        ...
        buf = unsafe { buf.add(24) };
        let vc0 = crc_shift(crc0, klen * 4 + 8);
