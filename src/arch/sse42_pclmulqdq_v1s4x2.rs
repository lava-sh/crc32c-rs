...
pub unsafe fn crc32c(mut crc0: u32, mut buf: *const u8, mut len: usize) -> u32 {
    ...
    if len >= 80 {
        ...
        let vc0 = crc_shift(crc0, klen * 3 + blk * 16);
        let vc1 = crc_shift(crc1, klen * 2 + blk * 16);
        let vc2 = crc_shift(crc2, klen + blk * 16);
        let vc3 = crc_shift(crc3, blk * 16);
        let vc = mm_extract_epi64::<0>(_mm_xor_si128(
            _mm_xor_si128(vc0, vc1),
            _mm_xor_si128(vc2, vc3),
        ));
